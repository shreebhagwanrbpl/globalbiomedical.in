...
export default function ProductsClient({ initialProducts = [], district = null, city = null }) {
  ...
  // Sync state if initialProducts changes on navigation
  useEffect(() => {
  ...
  // Zero-Delay Live Sync (Window Focus + 3s Interval)
        ...
        if (data && data.success && Array.isArray(data.products)) {
          setProducts((prev) => {
            if (
              ...
              prev.every((p, idx) => p.id === data.products[idx]?.id && p.isPublished === data.products[idx]?.isPublished)
            ) {
              return prev;
            }
  ...
  return (
    <Profiler id="ProductsLayout" onRender={onRenderCallback}>
      ...
      <section className="section-padding bg-white">
        ...
        <div className="max-w-2xl mx-auto mt-6 lg:mt-10 px-4 lg:px-0 relative">
          ...
          <input
            ...
            className="w-full h-16 pl-14 pr-5 rounded-2xl border border-slate-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-600"
          />
        </div>
            ...
            <div className="sticky top-0 -mx-6 pt-6 px-6 pb-3 bg-white z-20 border-b border-slate-100 mb-4 h-[116px]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-10 pl-10 pr-4 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-sky-600 focus:bg-white transition-all"
                />
              </div>
            </div>
      ...
      {showTopButton && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 z-50 w-14 h-14 rounded-full bg-sky-700 text-white shadow-2xl hover:bg-sky-800 hover:scale-110 transition flex items-center justify-center"
        >
          <ChevronUp size={24} />
        </button>
      )}
